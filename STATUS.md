...
- [x] **Themes registry** — single file `PolkadotAppTheme.kt` in `io.pcf.polkadotapp.designsystem.themes`. Enum with bare cases plus member functions `colors(): PolkadotColorsPalette` and `typography(): PolkadotTypography`, each an exhaustive `when (this)` over the enum. Adding a theme is a compile-time check on both branches. Construction is lazy — palettes/typographies allocate only when their resolver is called, so non-selected themes cost nothing at enum-load time. Plus `companion object { val DEFAULT = <first case> }`.

  Today: one case (`Default` → `PolkadotDefaultPalette()` / `PolkadotDefaultTypography()`). Spacings/radii/borders stay global — not theme-scoped in source — so they're absent from the registry. Emitter at `platforms/compose/themes.js` is plain Node + `fs.writeFileSync` (no Style Dictionary involvement — the registry is derived from `colors.themes` / `typography.themes` config, not from the token tree). Index-based pairing rule: the i-th colors theme pairs with the i-th typography theme; shorter list falls back to index 0. Consumer-side wiring (read selection → call `selection.colors()` / `selection.typography()` inside the `PolkadotTheme` composable wrapper) is deferred per user — only the mechanism ships in this version.
...
- [ ] Next tag `0.1.8` to publish: adds `PolkadotAppTheme.kt` (enum-with-factories registry pairing colors + typography). Consumer's `PolkadotTheme` wrapper still hardcodes `PolkadotDefaultPalette()`/`PolkadotDefaultTypography()` until that side is wired separately.
