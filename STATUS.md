...
**Last updated**: 2026-05-18 — pick up here with full context: lib at `0.1.9` (latest tag; adds `PolkadotAppTheme` enum with stable `id` per case + `fromId(id)` companion for prefs round-trip, on top of `0.1.8`'s themes-registry mechanism). AGP 8.12.3 + Kotlin 2.2.21 + compileSdk 36 unchanged since `0.1.4`. **Consumer is still pinned to `0.1.7`** — the design-system migration was merged to `master` via `[PANS-2683] Fully migrated on Design System tokens` (PR #596), but the multi-theme injection scaffolding hasn't been wired up yet. `PolkadotTheme` still hardcodes `PolkadotDefaultPalette()` / `PolkadotDefaultTypography()`. iOS template scaffolding shipped 2026-05-15 (`platforms/swift/`). Dimensions prefix-stripped vs colors verbatim is intentional (see "Naming conventions").
...
- [x] **Themes registry** — single file `PolkadotAppTheme.kt` in `io.pcf.polkadotapp.designsystem.themes`. Enum cases carry a stable `val id: String` (sourced from `selectionKey` in `platforms/compose/colors.js` — explicit so renaming the Kotlin case name or the concrete `Palette` class doesn't break persistence) plus member functions `colors(): PolkadotColorsPalette` and `typography(): PolkadotTypography`, each an exhaustive `when (this)` over the enum. Adding a theme is a compile-time check on both branches. Construction is lazy — palettes/typographies allocate only when their resolver is called, so non-selected themes cost nothing at enum-load time. Companion: `val DEFAULT` (first case) + `fun fromId(id: String?): PolkadotAppTheme` that falls back to `DEFAULT` on miss (matches the `Language.fromCode` pattern in consumer).

  Today: one case (`Default` → `id = "polkadotDefault"` → `PolkadotDefaultPalette()` / `PolkadotDefaultTypography()`). The id matches iOS's `ThemeSelection.polkadotDefault` rawValue so both platforms can share the same persistence key. Spacings/radii/borders stay global — not theme-scoped in source — so they're absent from the registry. Emitter at `platforms/compose/themes.js` is plain Node + `fs.writeFileSync` (no Style Dictionary involvement — the registry is derived from `colors.themes` / `typography.themes` config, not from the token tree). Index-based pairing rule: the i-th colors theme pairs with the i-th typography theme; shorter list falls back to index 0. `themes.js` throws at build time if `selectionKey` is missing on any colors theme. Consumer-side wiring (read selection → call `selection.colors()` / `selection.typography()` inside the `PolkadotTheme` composable wrapper) is still deferred — only the lib mechanism ships in `0.1.8`/`0.1.9`.
...
Big-bang migration **merged to `master`** via `[PANS-2683] Fully migrated on Design System tokens` (PR #596). Consumer pinned to `0.1.7` (variable-font bundle); full Kotlin compile was green at `0.1.5` and the changes since have been incremental (font fixes only). The earlier `StatementTransportEvent.kt` IR-evaluator crash went away after the lib's AGP/Kotlin downgrade in `0.1.4`/`0.1.5`. The themes registry (`0.1.8` / `0.1.9`) is not yet wired up — `design/theme/Theme.kt` still constructs `PolkadotDefaultPalette()` / `PolkadotDefaultTypography()` directly.
...
- [x] **Merged to `master`** via `[PANS-2683] Fully migrated on Design System tokens` (PR #596). All migration commits from the branch are now in master.
- [ ] **Bump consumer to `0.1.9`** + wire `PolkadotTheme` to consume the registry. Today `design/theme/Theme.kt` still hardcodes `PolkadotDefaultPalette()` / `PolkadotDefaultTypography()`. To unlock theme switching: pass a `PolkadotAppTheme` selection in (via param, CompositionLocal, or Hilt-EntryPoint observed source — earlier attempts at all three got reverted; pick whichever fits the team conventions) and call `selection.colors()` / `selection.typography()`. Persistence layer (`SingleValueStorage<String>` keyed off `PolkadotAppTheme.id` + `fromId` for read-back) is the natural Language-picker-shaped follow-up.
...
- `platforms/compose/themes.js` — emits `out/android/themes/PolkadotAppTheme.kt` (enum with `colors()` / `typography()` member fns using exhaustive `when (this)`). Plain `fs.writeFileSync` (no Style Dictionary). Reads `colors.themes` + `typography.themes` lists and pairs them index-by-index; the enum case name comes from the colors-class name with `Polkadot` prefix and `Palette` suffix stripped (so `PolkadotDefaultPalette` → `Default`). Cleans up the stale `PolkadotThemes.kt` from the earlier two-file iteration on each build, so accidental copies don't end up in the distribution lib.
...
- `gradle/libs.versions.toml` — pin `design-system = "0.1.7"` on master today (latest published lib tag is `0.1.9`; consumer needs a bump to access `PolkadotAppTheme`).
